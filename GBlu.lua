...
Notification:Notify(
    {Title = "RayFlare", Description = "Made by TheNcore#9999"},
    {OutlineColor = Color3.fromRGB(0, 166, 58),Time = 5, Type = "image"},
    {Image = "http://www.roblox.com/asset/?id=6023426923", ImageColor = Color3.fromRGB(0, 166, 58)}
)

local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/QuerNukerHub/UI-Library/main/Green%20Lord%20Library%20.txt"))()
local Window = Library.CreateLib("RayFlare ( Universal )", "Serpent")
...
local OB = Esp:NewSection("Visuals")
...
Section:NewButton("RayFlare.lua", "RayFlare.lua", function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/HalloweenKills/Load/main/lua"))();
end)

Section:NewButton("RayFlare Beta", "Beta", function()
    local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/HalloweenKills/UI/main/Bracket%20V4%20(%204%20).txt"))()

local Window, MainGUI = Library:CreateWindow("RayFlare.lua [ Beta ] ( Da Hood Bot Aim Trainer )")

local Tab1 = Window:CreateTab("Aiming")
local Tab2 = Window:CreateTab("Visuals")
local Tab3 = Window:CreateTab("Macro")
local Tab4 = Window:CreateTab("Anti lock")

local Groupbox1 = Tab1:CreateGroupbox("Aimbot", "Left")
local Groupbox2 = Tab1:CreateGroupbox("Camlock", "Right")

local Groupbox3 = Tab2:CreateGroupbox("Visual", "Left")

local Groupbox4 = Tab3:CreateGroupbox("Macro", "Left")

local Groupbox5 = Tab4:CreateGroupbox("Anti lock", "Left")


local ExampleToggle = Groupbox1:CreateToggle("Enable", function(state)
    if state then
...
local keytoclick = Enum.KeyCode.Q

local ExampleToggle1 = Groupbox1:CreateToggle("keybind [ Q ]", function(state)
    if state then
...
local prediction = 6.612

local ExampleSlider = Groupbox1:CreateSlider("Prediction", 0, 8.682, 0, function(value)
    prediction = value
end)

local function PredictTargetPosition(targetPos, targetVel, bulletSpeed, predictionVal)
    local predictedPos = targetPos + (targetVel * predictionVal)
    return predictedPos
end
...
local function Aimbot()
    ...
    local predictedPosition = PredictTargetPosition(targetPosition, targetVelocity, bulletSpeed, prediction)
...
local ExampleDropdown = Groupbox1:CreateDropdown("Aim Part", {"Head", "UpperTorso", "HumanoidRootPart", "LowerTorso"}, function(state)
    aimPart = state
end)
...
local function Aimbot()
    ...
    if targetPart then
        print("Aiming at:", aimPart)

    else
        print("Could not find target part:", aimPart)
    end
end

local playerName 

local ExampleToggle3 = Groupbox1:CreateToggle("Notification", function(state)

    if state then
        local lockedOnPlayer = AimlockTarget
        if lockedOnPlayer then
            playerName = lockedOnPlayer.Name
        end
    else
        playerName = nil 
    end
...
local ExampleToggle4 = Groupbox1:CreateToggle("View Player", function(state)
    if state and AimlockTarget and AimlockTarget.Character then
        local playerPart = AimlockTarget.Character:FindFirstChild("Head") 
        if playerPart then

            local viewPlayerPart = Instance.new("Part")
            viewPlayerPart.Name = "ViewPlayer"
            viewPlayerPart.Position = playerPart.Position
            viewPlayerPart.Size = Vector3.new(2, 2, 2)
            viewPlayerPart.BrickColor = BrickColor.new("Really red")
            viewPlayerPart.Transparency = 0.5
            viewPlayerPart.Anchored = true
            viewPlayerPart.CanCollide = false
            viewPlayerPart.Parent = workspace

            coroutine.wrap(function()
                wait(2)
                viewPlayerPart:Destroy()
            end)()
        end
    end
end)
    ...
    if AimlockTarget and AimlockTarget.Character then
        ...
        if AimlockTarget ~= nil then
            ...
            if MousePressed ~= false then 
                ...
                if playerName then
                    SendNotification("Locked on: " .. playerName)
                end
            end
        end
    end
...
local ExampleToggle = Groupbox2:CreateToggle("Enable", function(state)
    if state then
        getgenv().OldAimPart = "HumanoidRootPart"
        getgenv().AimPart = "HumanoidRootPart"
        getgenv().AimlockKey = "q"
...
local ExampleToggle1 = Groupbox2:CreateToggle("keybind [ Q ]", function(state)
    if state then
        local vim = game:GetService("VirtualInputManager")
        vim:SendKeyEvent(true, keytoclick, false, game)
    end
...
local ExampleSlider = Groupbox2:CreateSlider("Prediction", 0, 6.283, 0, function(value)
    prediction = value
end)
...
local ExampleDropdown = Groupbox2:CreateDropdown("Aim Part", {"Head", "UpperTorso", "HumanoidRootPart", "LowerTorso"}, function(state)
    aimPart = state
end)
...
local ExampleToggle3 = Groupbox2:CreateToggle("Notification", function(state)
...
local ExampleToggle4 = Groupbox2:CreateToggle("View Player", function(state)
    if state and AimlockTarget and AimlockTarget.Character then
        local playerPart = AimlockTarget.Character:FindFirstChild("Head") 
        if playerPart then
...
local ExampleToggle = Groupbox3:CreateToggle("Example toggle", function(state)
   print(state)
end)

local ExampleToggle = Groupbox3:CreateToggle("Enable ESP", function(state)
   ESP.Enabled = arg
end)

local ExampleToggle1 = Groupbox3:CreateToggle("ESP Boxes", function(state)
   ESP.Boxes = arg
end)

local ExampleToggle2 = Groupbox3:CreateToggle("ESP Names", function(state)
   ESP.Names = arg
end)

local ExampleToggle3 = Groupbox3:CreateToggle("ESP TeamMates", function(state)
   ESP.TeamMates = arg
end)

local ExampleToggle4 = Groupbox3:CreateToggle("ESP TeamColor", function(state)
   ESP.TeamColor = arg
end)

local ExampleToggle5 = Groupbox3:CreateToggle("ESP Face Camera", function(state)
   ESP.FaceCamera = arg
end)

local ExampleToggle6 = Groupbox3:CreateToggle("ESP Tracers", function(state)
   ESP.Tracers = arg
end)

local ExampleToggle = Groupbox3:CreateToggle("ForceField Body Chams", function(state)
       if callback then
        game.Players.LocalPlayer.Character.LeftHand.Material = "ForceField"
        game.Players.LocalPlayer.Character.RightHand.Material = "ForceField"
        game.Players.LocalPlayer.Character.LeftLowerArm.Material = "ForceField"
...
local ExampleToggle1 = Groupbox3:CreateToggle("BodyCham Color", function(state)
   game.Players.LocalPlayer.Character.LeftHand.Color = color
    game.Players.LocalPlayer.Character.RightHand.Color = color
    game.Players.LocalPlayer.Character.LeftLowerArm.Color = color
    game.Players.LocalPlayer.Character.RightLowerArm.Color = color
...
local ExampleToggle2 = Groupbox3:CreateToggle("ForceField Gun Chams", function(state)
  if callback then
        local Client = game.GetService(game, "Players").LocalPlayer
        Client.Character:FindFirstChildOfClass("Tool").Default.Material = Enum.Material.ForceField
        Client.Character:FindFirstChildOfClass("Tool").Default.BrickColor  = BrickColor.new(255, 255, 255)
...
local ExampleToggle3 = Groupbox3:CreateToggle("GunChams Color", function(state)
   game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool").Default.BrickColor = BrickColor.new(color)
end)

local ExampleToggle = Groupbox4:CreateToggle("Damage Fix", function(state)
...
local ExampleToggle2 = Groupbox4:CreateToggle("CFrame Speed (C)", function(state)
   repeat
        wait()
    until game:IsLoaded()
...
local ExampleSlider = Groupbox4:CreateSlider("CFrame Speed", 0, 9.0, 0, function(value)
...
local ExampleToggle = Groupbox5:CreateToggle("Jitter", function(state)
...
local ExampleToggle1 = Groupbox5:CreateToggle("Spinbot", function(state)
...
local ExampleToggle2 = Groupbox5:CreateToggle("Blatant Spinbot", function(state)
...
local ExampleToggle3 = Groupbox5:CreateToggle("SlingShot AA (ASS)", function(state)
...
local ExampleToggle4 = Groupbox5:CreateToggle("Super Blatant FlySpin AA", function(state)
...
local ExampleToggle5 = Groupbox5:CreateToggle("Normal AA", function(state)
...
local ExampleToggle6 = Groupbox5:CreateToggle("Normal AA Speed", function(state)
   getgenv().antilockspeed = state
end)

MainGUI:SetMain(Main)
end)

local speed = Window:NewTab("Macro")
local rs = speed:NewSection("Macro:")
